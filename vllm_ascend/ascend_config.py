...
class BlasstConfig:
    ...
    Controls the BlasstAttentionScore (BlasST) branch:
      - enabled: route eligible attention calls (TND layout, causal-only
        sparse_mode 3, no sliding window / learnable sink) to the custom op
        instead of torch_npu FIA. Builds without the registered op fall back
        to FIA with a warning.
