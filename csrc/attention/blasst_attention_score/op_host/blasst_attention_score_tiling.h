...
// Cost-balanced task->core assignment perm bound (taskPerm is 4B/task in the
// tiling buffer). 1024 covers an 8k-token chunked-prefill rank at the E3
// packing (16 q rows/task); larger task counts fall back to round-robin.
constexpr int32_t MAX_PREFILL_ASSIGN_TASKS = 1024;
...
// Cost-balanced task->core assignment for the regular (round-robin) kernel
// path. Round-robin striding lands 6 uniform-cost tasks on some cores and 5
// on others (+12% makespan at 128 tasks / 24 cores, and causal tail widths
// skew it further); LPT packing with per-core count caps evens per-core work.
// taskPerm holds the packed task order; core i owns taskPerm[start_i, end_i).
TILING_DATA_FIELD_DEF(uint32_t, taskAssignFlag)
TILING_DATA_FIELD_DEF_ARR(uint32_t, MAX_CORE_NUM_FD, coreTaskStart)
TILING_DATA_FIELD_DEF_ARR(uint32_t, MAX_CORE_NUM_FD, coreTaskEnd)
TILING_DATA_FIELD_DEF_ARR(uint32_t, MAX_PREFILL_ASSIGN_TASKS, taskPerm)
...
public:
    ...
    // qS/qN task decomposition (mirrors blasst_attention_score_common.h; the
    // attribute-parsing gate also needs them before an FAInferTiling exists)
    static uint32_t GetQNBlockTile(uint32_t qSeqlen, uint32_t groupSize);
    static uint32_t GetQSBlockTile(uint32_t qSeqlen, uint32_t groupSize);
...
private:
    ...
    uint32_t NoSkipKvOf(const BatchParams &p, uint32_t s1Idx) const;
    void FillTaskAssignTilingData(FAInferTilingData &faTilingData);
    void fillCoreInfoForFlashDecode(FAInferTilingData &faTilingData, uint32_t groupSize,
                                    uint64_t perCoreTaskNum);
    void fillCoreInfoForPrefillFlashDecode(FAInferTilingData &faTilingData, uint32_t groupSize);
