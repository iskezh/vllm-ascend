...
template <
    ...
    MaskMode MASK_MODE_>
class BlockEpilogue<
    EpilogueAtlasA2OnlineSoftmax<LSE_MODE_, MASK_MODE_, float>,
    OutputType_,
    InputType_,
    MaskType_>
{
public:
    using DispatchPolicy = EpilogueAtlasA2OnlineSoftmax<LSE_MODE_, MASK_MODE_, float>;
    ...
    float sparseLamda;
    // Dense mode (sparseLamda <= -50, e.g. the -99 disabled sentinel): the
    // skip decision can never fire, so the whole sp machinery -- per-tile
    // diff/reduce/GetValue decision chain, the 1-byte GM flag store and the
    // cube-side DCCI + scalar reads -- is dead weight on the critical path.
    bool spEnabled = true;
    ...
    void init(Arch::Resource<ArchTag> &resource, float scaleValue_, float sparseLamda_ = -99.0f)
    {
        ...
        tempSpUb = resource.ubBuf.template GetBufferByByte<uint8_t>(LP_UB_TENSOR_OFFSET);
    }
    ...
    void StoreSpFlagResIntoHmb(AscendC::GlobalTensor<uint8_t> gSpNew, uint32_t gmOffsetSp)
    {
        ...
        // Dense mode: no GM flag store (the cube side skips the matching read
        // on the same runtime condition). The event pair stays so the
        // lpUb/tempSp UB aliasing choreography is unchanged.
        if (!spEnabled) {
    ...
    {
        // ElementInput is fp32 (ElementS = float): the MTE2 lands S directly in
        // the fp32 softmax buffer -- no half staging band, no upcast pass.
        // Block counts are 32B blocks of fp32 (FLOAT_BLOCK_SIZE = 8 elements).
        AscendC::DataCopy(
    ...
    {
        // S lands from GM already fp32 (ElementS = float, direct MTE2 into
        // lsUbTensor): scaling is a single in-place fp32 Muls. The former
        // fp16-S path needed a half-wide Muls + fp16->fp32 Cast + two pipe
        // barriers per tile, which measured as the dominant occ0 prefill cost
        // (NOCAST ablation: 256.4 -> 230.6us vs FIA 228.3).
        ...
        uint32_t columnNumRound, uint32_t dmUbOffsetCurCycle, uint32_t rowOffset, uint32_t isFirstStackTile, uint32_t pingpongFlag, uint8_t* sp_flag_temp)
    {
        if (isFirstStackTile) {
            ...
            *sp_flag_temp = 0;
            ...
            bool spRes = false;
            if (sp_flag == 1 && spEnabled) {
                // *** dm = lm - gm (compute diff to detect sparsity)
...
#ifdef BLASST_SPDBG  // SPDBG: print sp decision inputs (block0/rowOffset0 only, rate-limited)
...
private:
    ...
    AscendC::LocalTensor<uint8_t> tempSpUb;
};
}
